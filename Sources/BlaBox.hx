...
class BlaBox {
	...
	public function render(g: Graphics, x: Float = -1, y: Float = -1): Void {
		...
		if (x < 0) {
			x = (speaker == null) ? (0.5 * (TenUp4.width - width)) : sx - 0.3 * width;
			
			if (x + width > TenUp4.width) {
				x -= 30 + x + width - TenUp4.width;
			}
			if (x < 0) {
				x = 30;
			}
		...
		if (y < 0) {
			y = (speaker == null) ? (0.3 * (TenUp4.height - height)) : sy - 30 - height;
			if (y < 0) {
